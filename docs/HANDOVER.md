...
- **⏭️ NEXT UP: cut the v1.9.3 release.** All four delta-audit-#3 ports are **applied and committed** (see below); the version is bumped to 1.9.3 / build 22, `docs/RELEASES/v1.9.3.md` is written, and the DMG is built, notarized and stapled, and the appcast enclosure is filled in (byte length verified against the DMG). What's left: push, tag `v1.9.3`, `gh release create` with the DMG attached, then verify the live appcast + download URL and do a real in-app Sparkle upgrade from 1.9.2.
...
- **Pushed through `2bf60e4`** as of 2026-09-02. The five v1.9.3 commits above are local-only until the release commit lands.
