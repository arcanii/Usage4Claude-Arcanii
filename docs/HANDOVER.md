...
- **Current version:** v1.9.3 (2026-09-02) — see [RELEASES/](RELEASES/).

## Where we are right now (read if you're resuming a session)

- **⏭️ NEXT UP: nothing queued.** v1.9.3 shipped and is confirmed installed via in-app Sparkle (Bryan, 2026-09-02). Two follow-ups are deliberately parked, not forgotten:
  1. **Field-watch v1.9.3's stale-data banner.** It is the only new UI, and `bannerContentHeight = 37` in `UsageDetailView.dynamicHeight` is an *estimate*, not a measurement. The popover uses a rigid `.frame(height:)`, so if it is low, content clips off the bottom while the banner shows. Not yet observed on screen. Reproduce by dropping the network for a refresh cycle with the popover open.
  2. **The upstream PR** bundling our loopback-peer hardening with the `59f4efd` slot-collapse bug — see [UPSTREAM_CONTRIBUTIONS.md](UPSTREAM_CONTRIBUTIONS.md). **Decision 2026-09-02 (Bryan): hold until v1.9.3 has real-world time.** Draft for him; do not post.

- **Delta audit #3 ports — DONE 2026-09-02.** Upstream `1b42d1e`. Every claim was re-verified against the real fork code before editing (a 4-area fan-out with an adversarial refutation pass per area), which caught several things the audit had wrong — recorded in [UPSTREAM_PORT_AUDIT.md](UPSTREAM_PORT_AUDIT.md) under "Delta audit #3". Commits:
  - `b2c9606` — `five_hour` Optional + `usageDashboardUnavailable` (ports `494957d`, superseding `3878868`). **Two** compile breaks, not one: `ClaudeAPIResponseModels.swift:100` (`parseLimitData` takes a non-optional, dragging :154) as well as `DiagnosticManager.swift:326`. Also needed an all-null branch in `DiagnosticManager.analyzeResponse` that the audit missed entirely: once the field is Optional such a payload *decodes cleanly*, so the diagnostic would have flipped from a wrong "credentials" verdict to a wrong "everything works, utilization: n/a" verdict. 72 tests (+7).
  - `3460dd2` + `e83df2d` — `OAuthCallbackServer` deinit (from PR #77). Must be `listener?.cancel()`, **not** `deinit { stop() }` (the shape the other 4 deinits in this repo use): `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` makes `stop()` isolated, and that form is a hard error under Swift 6. `NWListener.cancel()` is declared in the Network module, which isn't built with that default, so it stays nonisolated.
  - `3233dd7` — settings-tab stability (layout half of `92223a2`). The audit listed only 4 edit sites across 2 files; the commit touches 3, adding `AboutView.swift` `Spacer()` → `Spacer(minLength: 0)`. Its copyright half is deliberately skipped (injects an upstream-author link, rolls the year back to 2025–2026). No `.strings` hunks: upstream's include de/fr, and the fork's line numbers are offset by 4.
  - `3cff48a` — stale-data banner (view half of `a23c801`). Landed with the **typed** `errorRequiresFullScreen` on `DataRefreshManager` rather than a view-layer string gate: `errorMessage` is a snapshot while `L.Error.*` re-resolves per access, so a language switch with a live auth error would have reclassified it as transient and hidden the re-auth action. Three defects beyond the upstream port had to be handled — a latch so the banner doesn't blink and resize the popover on every refresh, suppression after an account switch (`usageData` still holds the *previous* account's numbers), and an `!activeDisplayTypes.isEmpty` guard (custom display mode can be empty with non-nil data → a blank popover). `.cloudflareBlocked` and `.usageDashboardUnavailable` join the auth cases in the full-screen bucket, since their messages are instructions.

- **v1.9.3 shipped 2026-09-02.** Four upstream ports from delta audit #3 (detail in the bullet above). Headline: **a null `five_hour` no longer kills the whole usage fetch** — it was the only non-optional window, so one null aborted the decode on both fetch paths and, because `UsageHistoryStore.append` sits in the success branch only, wrote **nothing** to the append-only NDJSON on every tick (a permanent gap) while the built-in diagnostic blamed the user's credentials. Also: plans with no usage dashboard now get their own `UsageError.usageDashboardUnavailable` and a dedicated diagnostic verdict stating the credentials are valid; a transient failure keeps the cached rings behind a tappable "Showing cached data" banner instead of a full-screen error; `OAuthCallbackServer` releases its listener on dealloc; and the settings tab strip no longer clips. 72 tests (+7). Tag `v1.9.3`, single release commit `5d8af6a`. **Verified**: notarized + stapled, `stapler validate` passed, live appcast served v1.9.3/build 22, the served asset's `Content-Length` (7348482) matched the Sparkle enclosure and the local DMG byte-for-byte, and the **in-app 1.9.2 → 1.9.3 Sparkle upgrade installed** (Bryan, 2026-09-02).
  - **Not yet verified on screen:** the stale-data banner's rendered height, and the settings-tab layout change (which is hardening — the fork's About tab is ~64pt shorter than upstream's since `aefc4a3` removed the ko-fi buttons, so it likely already fit).
  - **Known limitation:** an account with no usage windows but *with* Extra Usage credit still fails the whole fetch rather than showing the Extra Usage row alone — `fetchUsage` merges `async let main` / `async let extra` and a throwing `main` discards `extra`. Unchanged by this release (such accounts already failed, just with a worse message) and upstream behaves identically.
...
- **v1.9.2 shipped 2026-09-02.** Patch, one fix: **rate-limit backoff**. A 429 previously changed nothing about the refresh cadence — the failure handler only set `errorMessage` and special-cased `sessionExpired` — so in smart/active mode the app kept polling every 60s against an endpoint asking for less traffic, continuously renewing the limit instead of letting it lapse. Worse, `refreshOnPopoverOpen()` forces smart mode to its fastest tier, so opening the popover *to look at the error* re-armed the loop. Now: `UsageError.rateLimited` carries the server's `Retry-After` (parsed by `ClaudeAPIService.retryAfterSeconds(from:)`, handling both RFC 9110 forms — delta seconds and HTTP-date); `DataRefreshManager` suppresses fetches until the window closes, with the guard in `fetchUsage()` — the single choke point every trigger shares (timer, popover, wake, account switch, manual), so no path bypasses it; `Retry-After` wins when supplied, clamped 30s–30min so a tiny value can't spin, else 2min doubling to a 30min ceiling; a successful fetch clears the window and resets escalation; and the popover no longer escalates the tier while limited. Also corrected this file's intro, which still described cookie scraping as the only auth path (misleading since v1.8.0). 65 tests. Tag `v1.9.2`, single release commit `0c0b794`. **Verified**: notarized + stapled, DMG byte length matched the enclosure, live appcast served v1.9.2/build 21, and the **in-app 1.9.1 → 1.9.2 Sparkle upgrade completed with the error resolved** (Bryan, 2026-09-02).
  - **Known gap:** the `Retry-After` parser has no permanent regression coverage — `ClaudeAPIService` isn't in the SwiftPM test target. It was verified against both RFC forms plus negative/malformed/absent inputs via a standalone harness. Extracting the parser into a testable helper would close this; folded into the backlog rather than churning a release.
