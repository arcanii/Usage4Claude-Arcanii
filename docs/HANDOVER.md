...
- **Unreleased on `main` (post-v1.9.0): widget weekly-model labels.** v1.9.0 fixed the popover label but the widget target still hardcoded `"Opus"`/`"Sonnet"` tiles, so the same Fable number rendered as "Opus" on the desktop. `UsageSnapshot` now carries optional `opusModelName`/`sonnetModelName`, populated by `UsageSnapshotBridge` and used by `LargeDashboardWidget` / `ExtraLargeWidget` with the old strings as fallback. The fields are **Optional and additive on purpose** — the widget may read a snapshot written by an older app mid-update, so absent keys must decode as nil. `UsageSnapshot.swift` was added to the SwiftPM test target with two guards (`UsageSnapshotTests`): legacy-snapshot decode and model-name round-trip. 65 tests.
