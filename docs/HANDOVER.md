...
- **⏭️ NEXT UP (2026-09-02): delta audit #3 is done, the ports are NOT yet applied.** Upstream moved to `1b42d1e` (2026-09-01, still tagged v3.3.0) — 11 commits past the previous baseline `25bdf2b`. Six areas were assessed and every port/partial recommendation checked came back `holds=true`. Full write-up with exact file:line evidence is in **[UPSTREAM_PORT_AUDIT.md](UPSTREAM_PORT_AUDIT.md) → "Delta audit #3"**. Do these, then cut **v1.9.3**:
  0. **Do items 1+2 as ONE commit — port `494957d` as the superset** (it contains the optional-`five_hour` change and adds the dashboard diagnosis). They touch the same lines. **One compile-break to fix in the same commit:** `DiagnosticManager.swift:326` reads `usageData.five_hour.utilization`; note `createReportForSuccess` is implicit-return so a `let` needs an added `return`. Also: an affected account writes **nothing** to the append-only NDJSON history every tick (append is in the success branch only) — a permanent gap, and the built-in diagnostic misreports it as a credentials problem.
  1. **`five_hour` must be Optional** (upstream `3878868`) — **high, total-fetch failure.** `Helpers/ClaudeAPIResponseModels.swift:47` is the only non-optional window; a `"five_hour": null` throws `valueNotFound` and kills the whole decode on BOTH paths (`ClaudeAPIService.swift:277-283` and `:629`). ~3 lines + a diagnostics fix; everything downstream already tolerates nil and there are zero force-unwraps of `fiveHour`. Adapt to the fork's init (`fiveHour:sevenDay:legacyOpus:legacySonnet:scopedWeeklyModels:extraUsage:`), NOT upstream's `weeklyModels:`.
  2. **Plans without a usage dashboard** (upstream `494957d`) — **high.** All-null windows (Free Tier; Team/Enterprise without the member dashboard) surface as `decodingError` → "check if your credentials are correct", which is wrong advice for valid credentials. Needs a distinct error case + string in the fork's 5 locales; `DiagnosticManager.swift:273` repeats the wrong verdict.
  3. **Stale-data banner** (upstream `a23c801`) — high. The fork already keeps `usageData` on failure; the missing half is the view layer. Wire the banner to the fork's existing `isAuthenticationError(_:)` — do NOT reintroduce upstream's string matching.
  4. **`OAuthCallbackServer` deinit** (from PR #77) — medium. `deinit { listener?.cancel() }`; a login window closed mid-flow can leak a bound port → spurious "port busy". SKIP the rest of that PR (see the audit doc).
  5. **Settings-tab stability** (upstream `92223a2`) — medium, cosmetic. `SettingsView.swift:81` needs `alignment: .top`; plus 3 `ToolbarButton.swift` layout edits.
  Already-have/skip: the incoming-connections build setting (fork is already post-revert), Codex badge, sponsorship links.

- **Two commits are unpushed** as of 2026-09-02: `009f1ab` (README roadmap ordering) and the v1.9.2 ship record. Run `git push origin main`.
